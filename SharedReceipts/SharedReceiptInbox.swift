...
actor SharedReceiptInbox {
    ...
    func pendingEntries() throws -> [SharedReceiptEntry] {
        ...
        for child in try FileManager.default.contentsOfDirectory(at: directory, includingPropertiesForKeys: [.isDirectoryKey, .isSymbolicLinkKey]) {
            // Another process may still be publishing this batch. Only a
            // completed, atomically renamed directory is safe to read.
            ...
            case .intent(let input):
                if let url = input.fileURL {
                    ...
                    guard bytes.count <= Self.maximumInMemoryFileBytes else {
                        throw SharedReceiptError(message: "This shared image is too large. Save it to Files, then share that file with Finances.")
                    }
    ...
    private func copyURL(_ source: URL, filename requestedName: String?, type requestedType: UTType?, id: UUID,
        ...
        guard source.isFileURL else { throw SharedReceiptError(message: "Share an image or PDF file, rather than a website link.") }
        ...
        if (try? source.resourceValues(forKeys: [.isSymbolicLinkKey]).isSymbolicLink) == true {
            throw SharedReceiptError(message: "Choose the receipt file itself, rather than a file link.")
        }
    ...
    private func supportedType(_ proposed: UTType?, filename: String) throws -> UTType {
        ...
        throw SharedReceiptError(message: "Finances accepts shared images and PDF receipts. Other file types can be attached from the transaction editor.")
    }
    ...
    private func regularFileValues(_ url: URL) throws -> URLResourceValues {
        ...
        guard values.isRegularFile == true, values.isSymbolicLink != true else {
            throw SharedReceiptError(message: "Choose individual receipt files, not folders or links.")
        }
        return values
    }
    ...
    private func ensureSpace(_ bytes: Int64, at url: URL) throws {
        ...
        if let available = attributes[.systemFreeSize] as? NSNumber, bytes > max(0, available.int64Value - 32 * 1024 * 1024) {
            throw SharedReceiptError(message: "There is not enough free space to prepare these receipts.")
        }
    }
    ...
    private var invalidEntry: SharedReceiptError {
        SharedReceiptError(message: "This shared receipt could not be opened. Its pending files have been preserved; share it again from the original app.")
    }
    private var changedSource: SharedReceiptError {
        SharedReceiptError(message: "A receipt changed while it was being copied. Please share it again.")
    }
}

struct SharedReceiptError: LocalizedError, Sendable {
    let message: String
