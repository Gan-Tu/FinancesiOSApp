...
## Remove refund and reimbursement tracking — September 13, 2026

Removed the tracking screens, payment-linking domain code, presentation caches, clock refresh hooks, store mutations, demo flags, help content, and feature-specific tests. Removed the register selection mode used only by the payment picker. Receipt attachments, scanning, transaction postings, and the shared SQLite/CloudKit/backup formats remain unchanged. Existing source metadata remains opaque and readable without a feature decoder or migration.
