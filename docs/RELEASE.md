...
The app and embedded `dev.gan.FinancesApp.iOS.Share` extension both require the App Group `group.dev.gan.FinancesApp.iOS` and CloudKit container `iCloud.dev.gan.FinanceApp`. Their signing profiles must include both capabilities. The extension uses the existing receipt service and can reconnect through the same iCloud identity. Receipt sessions are shared through the existing App Group's Keychain access group, protected as When Unlocked / This Device Only; legacy app-only sessions migrate on the next app open. Credentials are never written to receipt files or the picker catalog. `scripts/check_project.py` verifies matching groups and CloudKit markers, and the capability checker verifies both targets' project bookkeeping. Open the updated app once before sharing to publish current picker choices and receipt AI preferences.
