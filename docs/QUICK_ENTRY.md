...
Tracking uses versioned metadata in existing transaction source records, so ordinary Mac edits, iCloud sync, and full backups preserve it without a new CloudKit schema. Older Mac versions do not offer the tracking UI. Their Import as New Journal does not remap tracking links: cloned tracking shows Needs Attention and requires Mac metadata-remapping support. Use full restore to preserve tracking IDs; the original journal is unaffected by a clone.
