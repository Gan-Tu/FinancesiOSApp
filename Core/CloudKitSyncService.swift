...
struct CloudKitSyncRecordCodec: Sendable {
    ...
    func recordID(type: String, id: String) throws -> CKRecord.ID {
        guard (zoneID.zoneName == AssistantPreferences.zone ? Set([AssistantPreferences.domain]) : zoneID.zoneName == ReceiptPreferences.zone ? Set([ReceiptPreferences.domain]) : zoneID.zoneName == "FinancesAssist_v1" ? Set(["assist_account"]) : Self.domainTypes).contains(type), let uuid = UUID(uuidString: id), uuid.uuidString == id else {
